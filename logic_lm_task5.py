...
# 4. Self-Refinement
def self_refine(nl_query, prev_symbolic, feedback):
    api_key = "7fd022143e7b3bb14cd5e1bc4229c354af503e86d8fe6ffbd008a28e88d43cb4"
    url = "https://api.together.xyz/v1/chat/completions"
    headers = {"Authorization": f"Bearer {api_key}"}
    messages = [
        {"role": "system", "content": "The previous symbolic logic statement had an error. Please revise it to the correct format Predicate(entity)."},
        {"role": "user", "content": f"Original question: {nl_query}\nPrevious attempt: {prev_symbolic}\nError: {feedback}\nProvide a corrected symbolic logic statement:"}
    ]
    data = {
        "model": "meta-llama/Llama-3-8b-chat-hf",
        "messages": messages,
    ...
    if response.status_code == 200:
        ...
        match = re.search(r"(Animal|Vehicle)\([a-z]+\)", result)
        return match.group(0) if match else result
    return prev_symbolic

# 5. Pipeline with Self-Refinement Loop
solver = BackwardChainer(kb)

def logiclm_pipeline(nl_query, max_retries=2):
    symbolic = llm_translate(nl_query)
    for attempt in range(max_retries + 1):
        try:
            result = solver.solve(symbolic)
            return result, symbolic, attempt + 1  # Return attempt count
        except Exception as e:
            if attempt < max_retries:
                print(f"  Attempt {attempt + 1} failed: {str(e)}")
                print(f"  Refining symbolic representation...")
                symbolic = self_refine(nl_query, symbolic, str(e))
            else:
                return False, symbolic, attempt + 1
...
print("="*50)
print("Logic-LM Pipeline Results")
print("="*50)
for nl in nl_queries:
    result, symbolic, attempts = logiclm_pipeline(nl)
    print(f"Q: {nl}")
...
print("Revised changes include")
print("LLM Integration: Together AI Llama-3")
print("Self-Refinement: Iterative error correction")
print("Symbolic Reasoning: Backward chaining solver")
print("CIFAR-10 KB: Knowledge base from Task 3")
print("Three-Module Pipeline: Complete Logic-LM workflow")
print("="*50)